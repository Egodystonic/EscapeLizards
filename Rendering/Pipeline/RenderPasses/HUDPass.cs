...
namespace Ophidian.Losgap.Rendering {
	/// <summary>
	/// A render pass that renders model instances created on the added <see cref="SceneLayer"/>s, using (amongst other things), a
	/// <see cref="Camera"/> for input and a <see cref="SceneViewport"/> for output.
	/// </summary>
	public sealed partial class HUDPass : RenderPass {
		...
		private VertexShader glowVS;
		private RasterizerState rsState;
		private DepthStencilState dsState;
		private BlendState blendState;
		private bool presentAfterPass = true;
		private FragmentShader scaleDownShader, scaleUpShader, glowShader;
		private ShaderResourcePackage scaleDownShaderResPkg, scaleUpShaderResPkg, glowShaderVResPkg, scaleDownShaderResUnbindPkg, scaleUpShaderResUnbindPkg, glowShaderVResUnbindPkg;

		public VertexShader GlowVS {
			get {
		...
		public FragmentShader ScaleDownShader {
			get {
				lock (InstanceMutationLock) {
					return scaleDownShader;
				}
			}
			set {
				lock (InstanceMutationLock) {
					scaleDownShader = value;
					SetUpShaderResPackages();
				}
			}
		}

		public FragmentShader ScaleUpShader {
			get {
				lock (InstanceMutationLock) {
					return scaleUpShader;
				}
			}
			set {
				lock (InstanceMutationLock) {
		...
		public FragmentShader GlowShader {
			get {
				lock (InstanceMutationLock) {
					return glowShader;
				}
		...
		public HUDPass(string name) : base(name) {
